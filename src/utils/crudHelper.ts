...
export function constructUpdateQueryStringBasedOnParams(
  ...
  for (let i = 0; i < data.searchBy.length; i++) {
    ...
    columnValues.push(data.value[i].toString);
    counter++;
  }
  updateRecordQueryString = updateRecordQueryString.substring(
    0,
    updateRecordQueryString.length - 4
  );
  updateRecordQueryString += ` returning *`;
...
export function constructDeleteQueryStringBasedOnParams(
  ...
  let deleteRecordQueryString = `delete from ${data.tableName} WHERE`;
  for (let i = 0; i < data.searchBy.length; i++) {
    deleteRecordQueryString += ` ${data.searchBy[i]} = '${data.value[i]}' AND`;
  }
  deleteRecordQueryString = deleteRecordQueryString.substring(
    0,
    deleteRecordQueryString.length - 4
  );
  deleteRecordQueryString += ` returning *`;
  return deleteRecordQueryString;
}
